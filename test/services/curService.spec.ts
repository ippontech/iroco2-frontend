...
describe("CurService", () => {
  ...
  describe("getFileExtension", () => {
    it("should extract extension when file has standard extension", async () => {
      // given
      ...
      // then
      expect(mockFetch).toHaveBeenCalledWith(
        `${PRESIGNED_URL_ENDPOINT}/pdf`,
        expect.any(Object),
      );
    });

    it("should extract last extension when file has multiple dots", async () => {
      // given
      ...
      // then
      expect(mockFetch).toHaveBeenCalledWith(
        `${PRESIGNED_URL_ENDPOINT}/gz`,
        expect.any(Object),
      );
    });

    it("should return null when filename has no extension", async () => {
      // given
    ...
    it("should handle uppercase extensions when uploading", async () => {
      // given
      const file = new File(["content"], "document.CSV", { type: "text/csv" });
      mockFetch.mockResolvedValue(MOCK_SERVER_RESPONSE);
      mockGlobalFetch.mockResolvedValue({ ok: true });
